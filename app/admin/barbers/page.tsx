...
export default function BarbersTeamPage() {
  ...
  // ================= REALTIME SYNC (Pentru Tabelul Echipei) =================
  useEffect(() => {
    const supabase = createClient();

    const channel = supabase
      .channel("realtime-admin-team-page")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "profiles" },
        () => {
          fetchBarbersAndStats(false); // Refresh tăcut (fără loading screen)
        },
      )
      .on(
        "postgres_changes",
        ...
        () => {
          fetchBarbersAndStats(false); // Refresh tăcut pentru a actualiza counterele
        },
      )
      .subscribe();
  ...
  const fetchBarbersAndStats = async (showLoader = true) => {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-10">
        <div className="bg-cyan-500/10 border border-cyan-500/20 p-6 rounded-[2rem] flex items-center gap-5 relative overflow-hidden group shadow-lg">
          ...
          <div className="relative z-10">
            ...
            <p className="text-4xl font-black text-white">{totalBarbers}</p>
          </div>
        </div>
        ...
        <div className="bg-green-500/10 border border-green-500/20 p-6 rounded-[2rem] flex items-center gap-5 relative overflow-hidden group shadow-lg">
          ...
          <div className="relative z-10">
            ...
            <p className="text-4xl font-black text-white">{activeBarbers}</p>
          </div>
        </div>
        ...
        <div className="bg-red-500/10 border border-red-500/20 p-6 rounded-[2rem] flex items-center gap-5 relative overflow-hidden group shadow-lg">
          ...
          <div className="relative z-10">
            ...
            <p className="text-4xl font-black text-white">{suspendedBarbers}</p>
          </div>
        </div>
      </div>
        ...
        <div className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative z-10 shrink-0">
          ...
          <div className="relative w-full sm:w-96">
            ...
            <input
              ...
              className="w-full bg-black/40 border border-white/10 focus:border-cyan-400 rounded-2xl pl-11 pr-4 py-3.5 text-white placeholder:text-slate-500 font-medium outline-none transition-all shadow-inner text-sm"
            />
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead className="sticky top-0 z-20 bg-[#050505]/95 backdrop-blur-xl border-b border-white/20 shadow-sm">
              <tr className="text-slate-300 text-[10px] uppercase tracking-widest font-black">
                <th className="py-4 pl-6">Profil & Contact</th>
                <th className="py-4">Detalii Profesionale</th>
                <th className="py-4 text-center">Activitate</th>
                <th className="py-4 text-right pr-6">Acces Profil</th>
              </tr>
            </thead>
                ...
                <tr>
                  <td colSpan={4} className="py-16 text-center">
                    ...
                    <p className="text-sm text-slate-400 font-medium">
                      Nu am găsit niciun frizer care să corespundă căutării.
                    </p>
                  </td>
                </tr>
              ) : (
                    ...
                    >
                      ...
                      <td className="py-5 align-middle">
                        <div className="flex flex-col gap-1.5">
                          ...
                          >
                            <span className="text-slate-500 font-black uppercase tracking-widest mr-1">
                              Curs:
                            </span>
                      ...
                      <td className="py-5 align-middle text-center">
                        <div className="flex justify-center gap-6 bg-black/40 rounded-2xl py-2 px-4 border border-white/5 shadow-inner w-max mx-auto">
                      ...
                      <td className="py-5 text-right pr-6 align-middle">
                        <Link
                          ...
                          className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-cyan-400/30 text-white hover:text-cyan-400 transition-all text-sm font-black shadow-sm"
                        >
                          <svg
                            className="w-4 h-4"
                            fill="none"
