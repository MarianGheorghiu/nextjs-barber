...
export default function BarberSidebar() {
  ...
  const menuItems = [
    ...
    {
      name: "Programări (Lună)",
      href: "/barber/appointments/month",
      icon: (
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
  ...
  return (
    <>
      {/* Bară de Navigație pentru Mobil */}
      <div className="md:hidden sticky top-0 z-40 w-full bg-[#0a0a0a]/90 backdrop-blur-xl border-b border-white/10 p-4 flex justify-between items-center shadow-lg">
        <div className="text-xl font-extrabold text-white tracking-wider">
          Barber<span className="text-cyan-400">Pro</span>
        </div>
        <button
      ...
      <aside
        className={`fixed md:sticky top-0 left-0 h-screen bg-[#0a0a0a]/95 md:bg-[#0a0a0a]/90 backdrop-blur-xl border-r border-white/10 flex flex-col z-50 transition-all duration-300 ${isMobileOpen ? "translate-x-0 w-64 shadow-2xl" : "-translate-x-full w-64"} md:translate-x-0 ${isCollapsed ? "md:w-24" : "md:w-72"}`}
      >
        {/* Butoane de Toggle */}
        <button
          onClick={() => setIsMobileOpen(false)}
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center px-4 py-3.5 rounded-xl transition-all duration-300 cursor-pointer group ${isCollapsed ? "justify-center" : "gap-4"} ${isActive ? "bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 shadow-[0_0_15px_rgba(34,211,238,0.1)]" : "text-slate-400 hover:text-white hover:bg-white/5 border border-transparent"}`}
              >
                <svg
